...
const TABLES = {
  ...
  crafting_batches: 'crafting_batch',
  ...
  private_messages: 'my_private_messages',
  social_relationships: 'my_social_relationships',
  trade_sessions: 'my_trade_sessions',
  trade_offers: 'my_trade_offers',
  ability_unlock_rules: 'ability_unlock_rule',
  actor_talent_pools: 'actor_talent_pool',
  parties: 'party',
  party_members: 'party_member',
  guilds: 'guild',
  guild_members: 'guild_member',
  object_rules: 'object_rule',
  bank_configs: 'bank_config',
  vendor_restock_rules: 'vendor_restock_rule',
  profession_definitions: 'profession_definition',
  recipe_rules: 'recipe_rule',
  actor_professions: 'actor_profession',
  actor_learned_recipes: 'actor_learned_recipe',
  dialogue_nodes: 'dialogue_node',
  dialogue_choices: 'dialogue_choice',
  actor_dialogue_states: 'actor_dialogue_state',
  exit_rules: 'exit_rule',
  world_triggers: 'world_trigger',
  actor_trigger_states: 'actor_trigger_state',
  content_issues: 'content_issue',
  world_simulation_configs: 'world_simulation_config',
  admin_audit_log: 'admin_audit_log',
  admin_player_sanctions: 'admin_player_sanctions',
  admin_player_reports: 'admin_player_reports',
  admin_world_snapshots: 'admin_world_snapshots',
};
const SUBSCRIBED_TABLES = Object.values(TABLES).filter((tableName) => tableName !== 'command');

const PRIMARY_KEYS = { regions: 'name', quest_rules: 'quest_id', admin_role_assignments: 'profile_id', ability_unlock_rules: 'ability_id', actor_talent_pools: 'actor_id', object_rules: 'definition_id', vendor_restock_rules: 'vendor_stock_id', recipe_rules: 'recipe_id', exit_rules: 'exit_id' };
...
async function completeAiCommand(row) {
  ...
  if (!npc) return;
  if (plainRows('dialogue_nodes').some((node) => node.npc_id === npc.id && node.entry_node)) return;
...
class SpacetimeClient {
  ...
  async configureEngineRecord(tableName, payload) {
    try {
      await callReducer('configureEngineRecord', { tableName, payloadJson: JSON.stringify(payload) });
      return { data: true, error: null };
    } catch (error) { return { data: null, error }; }
  }

  async deleteEngineRecord(tableName, recordId) {
    try {
      await callReducer('deleteEngineRecord', { tableName, recordId });
      return { data: true, error: null };
    } catch (error) { return { data: null, error }; }
  }

  async validateWorldContent() {
    try { await callReducer('validateWorldContent', {}); return { data: true, error: null }; }
    catch (error) { return { data: null, error }; }
  }

  async applyAdminPatch(runId, operations, summary = '') {
    try {
      await callReducer('applyAdminPatch', {
        runId,
  ...
  async moderatePlayer(actorId, action, reason = '', durationSeconds = 0) {
    try { await callReducer('moderatePlayer', { actorId, action, reason, durationSeconds }); return { data: true, error: null }; }
    catch (error) { return { data: null, error }; }
  }

  async resolvePlayerReport(reportId, status) {
    try { await callReducer('resolvePlayerReport', { reportId, status }); return { data: true, error: null }; }
    catch (error) { return { data: null, error }; }
  }

  async saveWorldSnapshot(snapshotId, name, content) {
    try { await callReducer('saveWorldSnapshot', { snapshotId, name, contentJson: JSON.stringify(content) }); return { data: true, error: null }; }
    catch (error) { return { data: null, error }; }
  }

  async deleteWorldSnapshot(snapshotId) {
    try { await callReducer('deleteWorldSnapshot', { snapshotId }); return { data: true, error: null }; }
    catch (error) { return { data: null, error }; }
  }
}
...
export function disconnectSpacetime() {
  ...
  if (typeof document !== 'undefined') document.cookie = 'arkyv_spacetime_token=; Path=/; Max-Age=0; SameSite=Strict';
}
  ...
  readyPromise = new Promise((resolve, reject) => {
    let builder = DbConnection.builder()
      ...
      .onConnect((connected, connectedIdentity, issuedToken) => {
        ...
        if (typeof document !== 'undefined' && issuedToken) {
          const secure = window.location.protocol === 'https:' ? '; Secure' : '';
          document.cookie = `arkyv_spacetime_token=${encodeURIComponent(issuedToken)}; Path=/; SameSite=Strict${secure}`;
        }
