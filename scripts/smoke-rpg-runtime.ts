...
async function main() {
    ...
    try {
        ...
        await insert('npcs', [
            { id: enemyId, name: 'Runtime Enemy', alias: 'enemy', description: 'Drops guaranteed loot.', current_room: roomA, spawn_room: roomA, behavior_type: 'static', disposition: 'hostile', attack_on_sight: false, respawn_seconds: 0 },
        ...
        await insert('actor_stats', [
            { actor_id: heroId, stat_definition_id: 'health', base_value: 20, current_value: 20 },
            { actor_id: rivalId, stat_definition_id: 'health', base_value: 20, current_value: 20 },
            { actor_id: enemyId, stat_definition_id: 'health', base_value: 1, current_value: 1 },
        ]);
        ...
        await waitFor(() => [...connection.db.actor_stat.iter()].find((row: any) => row.id === `${heroId}::health`)?.currentValue < 20, 'hostile NPC attack');
        ...
        assert(recoveredHealth === 20, `Player recovered with ${recoveredHealth} health instead of base health.`);

        console.log('RPG runtime smoke test passed: authoritative rooms, safe/open PvP, chest pickup, patrol, hostile AI, defeat recovery, and enemy loot.');
    } finally {
